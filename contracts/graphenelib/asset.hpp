...
#include <graphenelib/symbol.hpp>
#include <tuple>
#include <limits>

#define CORE_SYMBOL string_to_symbol(4, "SYS")
   ...
   */
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         ...
         graphene_assert( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         graphene_assert( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
       ...
       */
      asset& operator+=( const asset& a ) {
         graphene_assert( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         graphene_assert( -max_amount <= amount, "addition underflow" );
         graphene_assert( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         graphene_assert( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
      ...
      GRAPHENE_SERIALIZE( asset, (amount)(symbol) )
   };
   ...
   */
   struct extended_asset : public asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         graphene_assert( a.contract == b.contract, "type mismatch" );
      ...
      GRAPHENE_SERIALIZE( extended_asset, (amount)(symbol)(contract) )
   };
